    ...
    async.sortBy(files, function (file, sort) {
      ...
      async.concatSeries(res.reverse(), function(file, cb) {
        fs.readFile(path + '/posts/' + file, function(err, data) {
          ...
          data.shift();
          var body = md(data[0]),
              post = {'title': props.title, 'timestamp': props.timestamp, 'props': props, 'body': body};
          cb(err, post);
        });
      }, function(err, posts) {
        if (err) throw err;
        ...
        jade.renderFile(path + '/templates/posts.jade', options, function(err, html) {
          write(path + '/public/index.htm', html);
        });
      });
    });
  });
      ...
      function(cb) {
        ...
        if (type == 'posts') {
          ...
          var body = md(data.join('\n\n')),
              options = {posts: [{'title': props.title, 'timestamp': props.timestamp, 'props': props, 'body': body}]};
        } else if (type == 'pages') {
        ...
        if (type == 'posts') {
          ...
          var writePath = path + '/public/pages/' + file.split('.')[0] + '.htm'
        };
